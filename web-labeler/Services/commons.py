...
def load_data(file_path, time_column=None, drop_columns=None):
    ...
    if 'Inv 1 AC-Leistung (W)' in data.columns and data[
        'Inv 1 AC-Leistung (W)'].dtype == object:  # if statement added. Be carefull
        data['Inv 1 AC-Leistung (W)'] = pd.to_numeric(data['Inv 1 AC-Leistung (W)'].str.replace('W', '', regex=False),
                                                      errors='coerce')
...
def preprocess_data_with_scaling(file_path, time_column, target_column, timesteps, scaler, drop_time_column=False):
    ...
    exclude_columns = ['hour', 'hour_cos', 'hour_sin', 'day', 'day_of_week', 'week_of_year', 'month', 'is_weekend',
                       'anomaly_label', time_column]
